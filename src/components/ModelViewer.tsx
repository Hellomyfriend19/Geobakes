  ...
  return (
    <Html center>
      {placeholderSrc ? (
        ...
        <div className="bg-cream-light/80 backdrop-blur-sm px-4 py-2 rounded-full border border-caramel/20 text-espresso font-mono text-xs whitespace-nowrap shadow-sm">
          {Math.round(progress)} %
        </div>
      )}
    </Html>
...
const ModelViewer = ({
  ...
  useEffect(() => {
    // Preload GLTF safely if possible
    try {
      useGLTF.preload(url);
    } catch (e) {
      console.warn("Failed to preload gltf", e);
    }
  }, [url]);
  ...
  return (
    ...
    >
      {showScreenshotButton && (
        <button
          onClick={capture}
          className="absolute border border-cream/20 hover:bg-cream/15 text-cream right-4 top-4 z-10 cursor-pointer px-4 py-2 rounded-xl shadows-into-light-regular text-lg bg-espresso-dark/65 backdrop-blur-sm shadow-md transition-all active:scale-95"
        >
          Capture PNG
        </button>
      )}
